...
def dijkstra(graph, start, end):
    ...
    waypoints = set()
    while True:
        (path_len, v, path) = heappop(queue)
        if v not in waypoints:
            path = path + [v]
            waypoints.add(v)
            if v == end:
                return path
            for next_item, edge_len in graph[v].items():
                heappush(queue, (path_len + edge_len, next_item, path))


def checkio(house, stephan, ghost):
    if stephan == 1:return 'N'
